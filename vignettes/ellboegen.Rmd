...
```{r}
IGN <- sapply(list(mod = mod, alt1 = alt1, alt2 = alt2, alt3 = alt3, alt4 = alt4), IGN)
# Sort (lowest to highest ignorance)
sort(IGN)
```

Based on `IGN` we would preferr the most complex model `mod` over
all alternative models (`alt1`, `alt2`, `alt3`, `alt4`).
From the alternative ones, `alt4` performs best in terms of ignorance.

We could also compare the "Cluster separation" returned by [`summary`](references/summary.html):

```{r}
summary(mod)$separation
summary(alt1)$separation
```

... or compare the outcome of the different models in a time series plot
([`tsplot`](references/tsplot.html)):
...
tsplot(list(mod = mod, alt4 = alt4, alt3 = alt3, alt2 = alt2, alt1 = alt1),
...
mod_gauss  <- foehnix(ff ~ rh, data = ell, family = "gaussian")
mod_cgauss <- foehnix(ff ~ rh, data = ell, family = "gaussian",
                      left = 0, tol = -Inf, maxit = 20)
mod_tgauss <- foehnix(ff ~ rh, data = ell, family = "gaussian",
...
mod_logis  <- foehnix(ff ~ rh, data = ell, family = "logistic")
mod_clogis <- foehnix(ff ~ rh, data = ell, family = "logistic",
                      left = 0, tol = -Inf, maxit = 20)
mod_tlogis <- foehnix(ff ~ rh, data = ell, family = "logistic",
                      left = 0, tol = -Inf, maxit = 20, truncated = TRUE)
```
...
get_IC <- function(x) {
    res <- c(N = length(x$filter_obj$good), logLik(x), AIC(x), BIC(x), IGN(x))
    return(res)
}
...
The more complex model `mod` shows a lower ignorance but, due to the
specification of the model, contains more missing values where no
classification is possible when data from the crest station is missing.
...
* take estimates from model `mod_logis`.

```{r}
# Combine results of both models:
prob <- merge(mod = fitted(mod), mod_logis = fitted(mod_logis))

# Number of missing entries
apply(prob, 2, function(x) sum(is.na(x)))

# For demonstration purposes: find rows where mod is missing
# while mod_logis returns a probability:
idx <- which(is.na(prob$mod) & !is.na(prob$mod_logis))[1L] + seq(-3, 3)
head(prob[idx, ])

# Combine the two (take probability from mod if available,
# else from mod_logis; both missing: NA).
