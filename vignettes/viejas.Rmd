...
head(data, n = 3)
# Check if we have a "zoo" time series object
is.zoo(data)
# And that the data are numeric
is.numeric(data)
```

Missing values in the
data set (`NA`s) are allowed and will be properly handled by all functions.
...
```{r plot_mod_2ddata, fig = TRUE, fig.width = 8, fig.height = 6}
# Grid definition for 2-D plot
idx  <- mod$filter_obj$good
prob <- fitted(mod)
cols <- cut(prob, breaks = seq(0, 1, by = 0.1), include.lowest = TRUE,
            labels = colorspace::diverging_hcl(10))
# Plot
with(mod$data[idx, ],
     plot(wind_speed ~ diff_temp, col = as.character(cols)[idx], pch = 19,
