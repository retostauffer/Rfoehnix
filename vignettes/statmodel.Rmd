...
with $\hat{\mathit{p}}$ as specified above (a-posteriori probability).
...
9. For $j > 1$: if $(\ell^{(j)} - \ell^{(j-1)}) < \text{tol}$ the likelihood
   could not have been improved (converged or stuck): stop EM algorithm. 
...
3. Given $\mathit{z}^{(0)}$ and $\mathbf{x}$: estimate logistic
   ...
   (see [iwls_logit](reference/iwls_logit.html)).
